...
async function checkToken(req: Request, res: Response, next: NextFunction) {
    ...
    if (AccessToken) {
        try {
            ...
            if (refresh) {
                res.cookie('id_token', refresh.idToken, { httpOnly: true, secure: false, sameSite: 'none',maxAge:3600*1000 });
                res.cookie('access_token', refresh.accessToken, { httpOnly: true, secure: false, sameSite: 'none',maxAge:3600*1000 });
                req.clientId = idUser;
                console.log(idUser);
        ...
        const refresh = await refreshToken(refresh_token)
        if (refresh) {
            res.cookie('id_token', refresh.idToken, { httpOnly: true, secure: false, sameSite:'lax' });
